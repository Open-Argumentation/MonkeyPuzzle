...
function add_argument(con_text = null, prem_text = null, con_id = null, prem_id = null) {
	/*
    ...
    Returns: a dict
	*/
	if((con_text != null || con_id != null) && (prem_text != null || prem_id != null)) {
...
function add_conflict(arg_text = null, arg_id = null, confilct_text = null, conflict_id = null) {
	/*
...
function get_resource(resource_id) {
	...
	*/
	for (let resource of sd["resources"]) {
...
function get_source(atom_id, resource_id) {
	...
	*/
	var sourced = [];
	var atom = get_atom(atom_id);
	for (let source of atom["sources"]) {
...
function saveSADFace(filename=null, filetype="json") {
	/*
	...
	*/
	...
	if ("dot" == filetype) {
		...
		f += '.json';
		text = prettyprint(JSON.parse(export_cytoscape()));
	} else {
		f += '.json';
